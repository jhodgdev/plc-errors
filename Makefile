...
usage:
	...
	@echo "  watch               -- Track files: plc-errors.cabal, src/* and run 'make build' on change"
	@echo "  test                -- Run cabal v2-test"
	@echo "  accept_pirs         -- Accept new PIR changes"
	@echo "  ghci                -- Run cabal v2-repl plc-errors"
...
watch: requires_nix_shell
	while sleep 1; do find src plc-errors.cabal | entr -cd make build; done
...
ghci: requires_nix_shell
	cabal v2-repl $(GHC_FLAGS) plc-errors
...
build_path = dist-newstyle/build/x86_64-linux/ghc-8.10.4.20210212/plc-errors-0.1
clear_build:
